function h = raincloud1xN_horizontal(y, y_base, config)
K = size(y,2);
if nargin<3
    config = [];
end
...
p.addParameter('func_summary', 'mean');

p.addParameter('add_dots', 0);
...
p.addParameter('add_bar', 0);
p.addParameter('bar_mid', 0.05);
p.addParameter('bar_width', 0.3);
p.addParameter('bar_linewidth', 1);
p.addParameter('bar_alpha', 0.6);
...
for k=1:K
    ax = y_base(k);    
%     config.is_right = is_right(k);
    h(k) = draw(ax, y(:,k), config.is_right(k), config.face_color(k,:), config);
    hold on;
end
...
func_summary = str2func(config.func_summary);
 
% bar
add_bar = config.add_bar;
bar_mid = config.bar_mid;
bar_width = config.bar_linewidth; % width
bar_linewidth = config.bar_linewidth; % line width
rect_alpha = config.bar_alpha;

% dots
add_dots = config.add_dots;
...
if add_patch
hpatch = patch(ypatch,xpatch,[1 1 1],'EdgeAlpha',1,'FaceColor',face_color,'EdgeColor','none','FaceAlpha',patch_alpha);    
hpatchl = line([dens_pos(1) dens_pos dens_pos(end) dens_pos(1)],box_mid +is_right*[0 dens 0 0], 'color',zeros(1,3));
% xl1 = rxp*[-1 1]+box_mid;
hold on;
end

sum_y = func_summary(y);
[~,ix]=min(abs(dens_pos - sum_y));
my = sum_y;
if 1 % add_mean
%     line([my box_mid+is_right*dens(ix)], [my box_mid])
    plot([my my], [box_mid+is_right*dens(ix) box_mid], 'color',face_color, 'linewidth', 3);
end

% % % if add_bar
% % %     mdy = .1*bar_width;
% % %     bar_mid = box_mid + -is_right*(bar_mid + bar_width);
...
if add_dots
    ...
    hscat = scatter(y, xp,dots_size,repmat(dots_color,N,1),'filled');
end
