function [se, boots] = se_median(u,N, dim)
...
if isempty(N), N   = 10000; end
if nargin<3, dim = 1; end
if size(u,1)<3, se = nan(1,size(u,2)); boots = []; return; end

se = nan(1,size(u,2));
for i=1:size(u,2)
[bootstat,bootsam] = bootstrp(N,@(x)nanmedian(x,dim),u(:,i));
mx = bootstat(:,1);
se(i) = std(mx);
