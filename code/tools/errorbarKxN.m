function hb = errorbarKxN(mx,ex,labels,colmap,barwidth,ylbl, alf)
% examples
% 
% % factorial
% close all;
...
%     colmap = repmat( (0:(1/(K-1)):1)',1,3);
    colmap = [0.8500    0.3250    0.0980;0    0.4470    0.7410; ];    
end
end
...
for i=1:N
    ...
    a(i) = median(ax);
    for k=1:K
        hb(k,i) = bar(ax(k),mx(k,i),wb,'FaceColor',colmap(k,:),'EdgeColor','k','linewidth',1,'basevalue',basevalue, 'FaceAlpha',alf(k));
        hold on;
    end
    for k=1:K        
